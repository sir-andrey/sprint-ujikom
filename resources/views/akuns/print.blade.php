...
<div class="head">
    ...
    <hr />
    <br>
    <p align="center" ><strong>Laporan Data Akun<br>Inventaris Sarana dan Prasarana SMKN 2 Cimahi </strong></p>
    <table class="styletable">
        ...
        <tbody>
        @foreach($andre_akun as $key => $data)
            <tr>
                <td>{{ $key+1 }}</td>
                <td>{{ $data->name }}</td>
                <td>{{ $data->email }}</td>
                <td>{{ $data->username }}</td>
                <td>{{ $data->level->andre_nama_level }}</td>
            </tr>
        @endforeach
